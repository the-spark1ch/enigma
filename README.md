...
* **Engine Kernel (`core.py`)**: Interfaces directly with the Windows API through 64-bit Registry abstractions (`KEY_WOW64_64KEY`), the Service Control Manager (`sc.exe`), and scheduled task interfaces (`schtasks.exe`). Process actions and thread-safe logging are synchronized via standard synchronization locks.
...
* **Transactional State Storage (`backup_state.json`)**: Preserves pre-remediation states—including original registry values, types (`REG_DWORD`, `REG_SZ`), and initial service startup modes (`Automatic`, `Manual`, `Disabled`)—within `%APPDATA%\Enigma`.
